...
void Hanger::ifStealthy()
{
	...
	else
	{
		...
		//Global::timer(30000000, 4, ranOutOfTime, pressedButton, options, timeDesc, Hanger::gShipAttacks, );
	}
}
...
void Hanger::gShipAttacks()
{
	...
	int random = ((rand() % 5) + 1);
	int finalDamage = Player::damage + gunBonus + random - GuardShip::guardEasyReturnDefense();
